...
SORT_FRAMES_BY_ROLL = True  # Sort by roll to ensure diverse roll picking

# ── Lever Arm Setup ──────────────────────────────────────────────────────────
# Initial guess for [X_OFFSET, Z_OFFSET] in meters
X_OFFSET_INIT = 0.0
Z_OFFSET_INIT = 0.0

# Y offset (along-track) is usually fixed or solved separately. 
# We hold this constant during the X/Z roll solver.
...
def _world_positions(params: np.ndarray, clicks: list[dict], y_off: float) -> tuple[np.ndarray, np.ndarray]:
    """Compute world Easting, Northing for a set of clicks given [x_off, z_off]."""
    x_off = float(params[0])
    z_off = float(params[1])
    
    E_list, N_list = [], []
    for c in clicks:
        # Removed img_dir from this call!
...
def _cost(params: np.ndarray, clicks_a: list[dict], clicks_b: list[dict], y_off: float) -> float:
    """Total world-coordinate variance for both corners given trial lever arms."""
    total = 0.0
    for clicks in (clicks_a, clicks_b):
        if len(clicks) < 2:
            continue
        # Removed img_dir from this call!
        E, N = _world_positions(params, clicks, y_off)
...
def collect_image_clicks_interactive(
    ...
    img_dir: str,
    interval: int = 1,
    sort_by_roll: bool = True
) -> tuple[list[dict], list[dict]]:
    
    from matplotlib.widgets import Button
    
    if sort_by_roll:
        df = df.sort_values("roll").reset_index(drop=True)
    ...
    def _draw():
        ...
        ax.set_title(
            f"Image {S['idx'] + 1}/{n_frames}  |  File: {fname}  |  Roll: {roll:+.2f}\u00b0\n"
    ...
    def _on_click(event):
        ...
        if mode == "A" and not S["done_a"]:
            ...
            print(f"  [A] File: {row['filename']} | Roll={row['roll']:+.2f}\u00b0 | u={rec['u']:.1f}, v={rec['v']:.1f}")
        elif mode == "B" and not S["done_b"]:
            S["clicks_b"] = [c for c in S["clicks_b"] if c["filename"] != row["filename"]]
            S["clicks_b"].append(rec)
            print(f"  [B] File: {row['filename']} | Roll={row['roll']:+.2f}\u00b0 | u={rec['u']:.1f}, v={rec['v']:.1f}")
        _draw()
...
def run_solver():
    ...
    print(f"Roll range: {roll_min:.2f}\u00b0 to {roll_max:.2f}\u00b0")
    ...
    if REUSE_SAVED_CLICKS and os.path.exists(CLICKS_SAVE_PATH):
        ...
        clicks_a, clicks_b = collect_image_clicks_interactive(df, IMAGE_DIR, FRAME_INTERVAL, SORT_FRAMES_BY_ROLL)
    ...
    print(f"\nStarting optimization (Initial X={X_OFFSET_INIT:.4f}, Z={Z_OFFSET_INIT:.4f})")
    print("Evaluating cost function... (This may take a minute due to image footprint mappings)")

    x0 = np.array([X_OFFSET_INIT, Z_OFFSET_INIT])
    cost_before = _cost(x0, clicks_a, clicks_b, LEVER_ARM_Y_FIXED)
    ...
    result = optimize.minimize(
        _cost, 
        x0=x0, 
        # Changed from: args=(clicks_a, clicks_b, LEVER_ARM_Y_FIXED, IMAGE_DIR),
        args=(clicks_a, clicks_b, LEVER_ARM_Y_FIXED),
        method="Nelder-Mead",
        options={"xatol": 1e-4, "fatol": 1e-6, "maxiter": 1000}
    )

    x_off_opt, z_off_opt = result.x[0], result.x[1]
    cost_after = result.fun

    print(f"\nOptimisation finished ({result.message})")
    print(f"  Cost after: {cost_after:.6e} m²")
    print(f"\n┌─ Solved Camera Lever Arms ─────────────┐")
    print(f"│  X_OFFSET = {x_off_opt:+.6f} m           │")
    print(f"│  Z_OFFSET = {z_off_opt:+.6f} m           │")
    print(f"└────────────────────────────────────────┘")
    ...
    for label, clicks, marker in [('A', clicks_a, 'o'), ('B', clicks_b, '^')]:
        ...
        E_bef, N_bef = _world_positions(x0, clicks, LEVER_ARM_Y_FIXED)
        E_aft, N_aft = _world_positions(result.x, clicks, LEVER_ARM_Y_FIXED)
    ...
    plt.suptitle(f"Camera X, Z Lever Arm Solver (Roll Test)\nSolved: X={x_off_opt:+.4f}, Z={z_off_opt:+.4f}")
